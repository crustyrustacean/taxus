...
<script>const ws = new WebSocket('ws://localhost:3000/__ws__');</script>
    ...
    fn test_live_reload_script_content() {
        ...
        assert!(yew_ssg_lib::serve::LIVE_RELOAD_SCRIPT.contains("WebSocket"));
        assert!(yew_ssg_lib::serve::LIVE_RELOAD_SCRIPT.contains("__ws__"));
        assert!(yew_ssg_lib::serve::LIVE_RELOAD_SCRIPT.contains("location.reload"));
    }
}
